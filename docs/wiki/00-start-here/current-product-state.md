...
finding. A local P-035/P-022 repair now passes focused authority, membership,
production-entrypoint, mode, accessibility, reflow, retry, reload, the full
347-test Node suite, and the full 79-test release-browser suite. Independent
review found the repaired authority boundaries ready after evidence refresh.
P-035 remains the current next card until the corrected tree is committed and
pushed, rebuilt as one immutable candidate, and passes live first-use proof.
The old public candidate remains unchanged and ineligible; do not treat local
acceptance as deployment.
