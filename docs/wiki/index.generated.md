<!-- GENERATED: run npm run wiki:generate. Do not edit by hand. -->

# ChopDot Wiki Index

Generated: 2026-07-15T10:37:45.052Z

This index is a read model for humans and agents. Edit source pages, not this file.

## Core Source References

- `product/product-principles.md`
- `product/story-map.md`
- `product/journey-review-plan.md`
- `product/cards.md`
- `product/journey-reviews/J-001-normal-pot-add-and-track-expenses.md`
- `product/journey-reviews/J-010-savings-circle-round.md`
- `product/journey-reviews/J-011-emergency-pot-privacy-flow.md`
- `product/design-references/chopdot-batch-1-2-consolidated-principles-2026-07-01.md`
- `.cursor/rules/chopdot-dot-programme.mdc`

## Wiki And ADR Pages

| Page | Status | Owner | Last reviewed | Tags | Related docs |
| --- | --- | --- | --- | --- | --- |
| [Use Repo Native Wiki](../../docs/adr/0001-use-repo-native-wiki.md) | current | Dev | 2026-07-05 | `adr`, `wiki` | `docs/wiki/README.md` |
| [KG Is Index Not Source Of Truth](../../docs/adr/0002-kg-is-index-not-source-of-truth.md) | current | Dev | 2026-07-05 | `adr`, `kg`, `agentops` | `docs/wiki/README.md`<br>`docs/agentops_kg_bridge.md` |
| [Thread Imports Are Evidence Not Truth](../../docs/adr/0003-thread-imports-are-evidence-not-truth.md) | current | Dev | 2026-07-06 | `adr`, `context-intake`, `codex-thread` | `docs/wiki/08-context-intake/context-intake.md`<br>`docs/adr/0002-kg-is-index-not-source-of-truth.md` |
| [Server-Derived Payment Actor Boundary](../../docs/adr/0004-server-derived-payment-actor.md) | current | Dev | 2026-07-14 | `adr`, `security`, `authentication`, `payment` | `docs/security/p025-security-foundation-crosswalk-2026-07-14.md`<br>`docs/security/universal-chop-core-security-architecture.md`<br>`docs/wiki/03-state-models/payment-state.md` |
| [Portable Product and Native Host Boundary](../../docs/adr/0005-portable-product-native-host-boundary.md) | current | Dev | 2026-07-14 | `adr`, `polkadot-native`, `portable-shell`, `product-sdk` | `.worktrees/portable-shell-trial/HOSTS.md`<br>`.worktrees/portable-shell-trial/plans/2026-07-14-polkadot-hosted-two-person-journey-v1.md`<br>`docs/wiki/05-polkadot-native/chopdot-dot-overview.md` |
| [Current Product State](../../docs/wiki/00-start-here/current-product-state.md) | current | Dev | 2026-07-14 | `start-here`, `status`, `routing` | `product/cards.md`<br>`product/generated/product-resume.md`<br>`product/generated/readiness-scorecard.md`<br>`product/journey-review-plan.md` |
| [How Agents Should Work](../../docs/wiki/00-start-here/how-agents-should-work.md) | current | Dev | 2026-07-05 | `start-here`, `agents`, `workflow` | `AGENTS.md`<br>`product/story-map.md`<br>`product/cards.md`<br>`product/journey-review-plan.md` |
| [What Is ChopDot](../../docs/wiki/00-start-here/what-is-chopdot.md) | current | Dev | 2026-07-05 | `start-here`, `product` | `product/product-principles.md`<br>`product/project-profile.md`<br>`product/story-map.md` |
| [Evidence Levels](../../docs/wiki/01-product-truth/evidence-levels.md) | current | Dev | 2026-07-05 | `product-truth`, `evidence` | `product/product-principles.md`<br>`product/generated/readiness-scorecard.md` |
| [Non Goals](../../docs/wiki/01-product-truth/non-goals.md) | current | Dev | 2026-07-05 | `product-truth`, `non-goals` | `product/product-principles.md`<br>`docs/CURSOR_CHOPDOT_2030_STRATEGIC_PLAN.md` |
| [Product Pillars](../../docs/wiki/01-product-truth/pillars.md) | current | Dev | 2026-07-05 | `product-truth`, `pillars` | `product/product-principles.md`<br>`product/taxonomy.md` |
| [Product Spine](../../docs/wiki/01-product-truth/product-spine.md) | current | Dev | 2026-07-05 | `product-truth`, `spine` | `product/product-principles.md`<br>`product/story-map.md` |
| [Community Fund Journey](../../docs/wiki/02-user-journeys/community-fund.md) | draft | Dev | 2026-07-14 | `journeys`, `community-fund` | `product/cards.md`<br>`product/story-map.md` |
| [Emergency Pot Journey](../../docs/wiki/02-user-journeys/emergency-pot.md) | current | Dev | 2026-07-14 | `journeys`, `emergency-pot`, `privacy` | `product/journey-reviews/J-011-emergency-pot-privacy-flow.md`<br>`product/design-references/chopdot-batch-1-2-consolidated-principles-2026-07-01.md` |
| [Normal Pot Journey](../../docs/wiki/02-user-journeys/normal-pot.md) | current | Dev | 2026-07-14 | `journeys`, `normal-pot` | `product/journey-reviews/J-001-normal-pot-add-and-track-expenses.md`<br>`product/journey-reviews/J-005-regular-pot-end-to-end-coherence.md` |
| [Savings Circle Journey](../../docs/wiki/02-user-journeys/savings-circle.md) | current | Dev | 2026-07-14 | `journeys`, `savings-circle` | `product/journey-reviews/J-010-savings-circle-round.md`<br>`product/cards.md` |
| [Settlement Journey](../../docs/wiki/02-user-journeys/settlement.md) | current | Dev | 2026-07-14 | `journeys`, `settlement` | `product/journey-reviews/J-002-normal-pot-settle-one-person.md`<br>`product/journey-reviews/J-004-no-app-friend-payment-link.md` |
| [Spend Capture Journey](../../docs/wiki/02-user-journeys/spend-capture.md) | draft | Dev | 2026-07-14 | `journeys`, `spend-capture` | `product/journey-reviews/J-000-dinner-split-pay-moment-capture.md`<br>`product/journey-reviews/J-009-receipt-capture-without-manual-first-entry.md` |
| [Native Session State Model](../../docs/wiki/03-state-models/native-session-state.md) | draft | Dev | 2026-07-14 | `state-model`, `polkadot-native` | `docs/wiki/05-polkadot-native/chopdot-dot-overview.md`<br>`docs/wiki/05-polkadot-native/native-boundaries.md`<br>`.cursor/rules/chopdot-dot-programme.mdc` |
| [Payment State Model](../../docs/wiki/03-state-models/payment-state.md) | current | Dev | 2026-07-14 | `state-model`, `payment` | `product/product-principles.md`<br>`docs/wiki/02-user-journeys/settlement.md`<br>`docs/adr/0004-server-derived-payment-actor.md`<br>`docs/security/p025-database-backed-actor-boundary-proof-2026-07-14.md` |
| [Pot Closeout State Model](../../docs/wiki/03-state-models/pot-closeout-state.md) | current | Dev | 2026-07-05 | `state-model`, `closeout` | `product/journey-reviews/J-003-normal-pot-close-record.md`<br>`product/product-principles.md` |
| [Privacy State Model](../../docs/wiki/03-state-models/privacy-state.md) | current | Dev | 2026-07-05 | `state-model`, `privacy` | `product/journey-reviews/J-011-emergency-pot-privacy-flow.md`<br>`docs/security/universal-chop-core-security-architecture.md` |
| [Batch 1 And 2 Principles](../../docs/wiki/04-design-quality/batch-1-2-principles.md) | current | Dev | 2026-07-05 | `design-quality`, `reference` | `product/design-references/chopdot-batch-1-2-consolidated-principles-2026-07-01.md` |
| [Effortless App Gate](../../docs/wiki/04-design-quality/effortless-app-gate.md) | current | Dev | 2026-07-05 | `design-quality`, `gate` | `product/product-principles.md`<br>`product/journey-review-plan.md` |
| [Money App Reference](../../docs/wiki/04-design-quality/money-app-reference.md) | current | Dev | 2026-07-05 | `design-quality`, `visual-reference` | `product/design-references/kast-premium-money-app-2026-06-30/README.md` |
| [Asset Hub](../../docs/wiki/05-polkadot-native/asset-hub.md) | current | Dev | 2026-07-15 | `polkadot-native`, `asset-hub`, `payment` | `product/journey-reviews/J-006-pas-test-wallet-payment.md`<br>`product/journey-reviews/J-007-wallet-payment-currency-checks.md`<br>`docs/wiki/03-state-models/payment-state.md` |
| [Bulletin](../../docs/wiki/05-polkadot-native/bulletin.md) | draft | Dev | 2026-07-14 | `polkadot-native`, `bulletin` | `.cursor/rules/chopdot-dot-programme.mdc`<br>`docs/wiki/03-state-models/pot-closeout-state.md` |
| [ChopDot Dot Overview](../../docs/wiki/05-polkadot-native/chopdot-dot-overview.md) | current | Dev | 2026-07-15 | `polkadot-native`, `chopdot-dot`, `portable-shell` | `.cursor/rules/chopdot-dot-programme.mdc`<br>`.cursor/rules/chopdot-dot-implementation.mdc`<br>`product/cards.md`<br>`docs/wiki/08-context-intake/w3s-open-source-map-2026-07-07.md` |
| [ChopDot Dot Smoke Lane](../../docs/wiki/05-polkadot-native/chopdot-dot-smoke.md) | current | Dev | 2026-07-15 | `polkadot-native`, `dot`, `smoke` | `docs/superpowers/plans/2026-07-07-chopdot-dot-smoke-lane.md`<br>`docs/wiki/05-polkadot-native/chopdot-dot-overview.md`<br>`docs/wiki/05-polkadot-native/native-boundaries.md`<br>`docs/wiki/08-context-intake/w3s-open-source-map-2026-07-07.md` |
| [Native Boundaries](../../docs/wiki/05-polkadot-native/native-boundaries.md) | current | Dev | 2026-07-14 | `polkadot-native`, `boundaries` | `.cursor/rules/chopdot-dot-programme.mdc`<br>`.cursor/rules/chopdot-dot-implementation.mdc` |
| [Statement Store](../../docs/wiki/05-polkadot-native/statement-store.md) | draft | Dev | 2026-07-14 | `polkadot-native`, `statement-store` | `.cursor/rules/chopdot-dot-programme.mdc`<br>`docs/wiki/03-state-models/native-session-state.md` |
| [Agent Build Loop](../../docs/wiki/06-agentops/build-loop.md) | current | Dev | 2026-07-06 | `agentops`, `build-loop` | `AGENTS.md`<br>`product/journey-review-plan.md`<br>`docs/CHOPDOT_OPERATING_LOOPS.md`<br>`docs/wiki/08-context-intake/context-intake.md` |
| [Agent Failure Modes](../../docs/wiki/06-agentops/failure-modes.md) | current | Dev | 2026-07-05 | `agentops`, `failure-modes` | `docs/AGENT_DOCTRINE_FEEDBACK.md`<br>`product/ai-manager/ai-product-management-adoption.md` |
| [Agent Review Loop](../../docs/wiki/06-agentops/review-loop.md) | current | Dev | 2026-07-05 | `agentops`, `review-loop` | `product/journey-review-plan.md`<br>`product/agent-reviews/P-009-normal-pot-agent-ui-audit.md` |
| [Hard Path QA](../../docs/wiki/07-quality/hard-path-qa.md) | current | Dev | 2026-07-05 | `quality`, `qa` | `product/product-principles.md`<br>`product/journey-review-plan.md` |
| [Release Checklist](../../docs/wiki/07-quality/release-checklist.md) | draft | Dev | 2026-07-05 | `quality`, `release` | `product/generated/readiness-scorecard.md`<br>`product/board-policy.md` |
| [Testing Strategy](../../docs/wiki/07-quality/testing-strategy.md) | current | Dev | 2026-07-05 | `quality`, `testing` | `product/journey-review-plan.md`<br>`product/generated/feature-to-test-coverage.md` |
| [Codex Thread 019f1ce5 Telegram Portable Shell](../../docs/wiki/08-context-intake/codex-thread-019f1ce5-telegram-portable-shell.md) | current | Dev | 2026-07-14 | `context-intake`, `codex-thread`, `portable-shell`, `telegram` | `.worktrees/portable-shell-trial/PORTABLE_SHELL_TRIAL.md`<br>`docs/wiki/08-context-intake/context-intake.md`<br>`product/cards.md` |
| [Context Intake](../../docs/wiki/08-context-intake/context-intake.md) | current | Dev | 2026-07-14 | `context-intake`, `agentops`, `wiki` | `docs/adr/0002-kg-is-index-not-source-of-truth.md`<br>`docs/adr/0003-thread-imports-are-evidence-not-truth.md`<br>`docs/wiki/06-agentops/build-loop.md` |
| [W3S Open Source Map 2026 07 07](../../docs/wiki/08-context-intake/w3s-open-source-map-2026-07-07.md) | current | Dev | 2026-07-14 | `context-intake`, `polkadot-native`, `w3s`, `parity` | `docs/superpowers/plans/2026-07-07-chopdot-dot-smoke-lane.md`<br>`docs/wiki/05-polkadot-native/chopdot-dot-smoke.md`<br>`docs/wiki/05-polkadot-native/chopdot-dot-overview.md`<br>`docs/wiki/05-polkadot-native/native-boundaries.md` |
| [ChopDot Wiki](../../docs/wiki/README.md) | current | Dev | 2026-07-06 | `wiki`, `agents`, `routing` | `product/product-principles.md`<br>`product/story-map.md`<br>`product/journey-review-plan.md`<br>`docs/wiki/08-context-intake/context-intake.md` |
| [Page Title](../../docs/wiki/wiki-template.md) | draft | Dev | 2026-07-05 | `template` |  |
