...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | J02 — Home / Orientation v1.4 | `journeys/02-home-orientation/v1.4-golden.html` | `578ff52775d7205b7f2262676f322807849e2f5788f219dc9eacbb9e5d3aec2f` | Attention-first/group-first Home; overall position; compact wallet context; group state + next action; contextual add; bottom navigation | `src/components/Home.tsx`; router shell in `src/App.tsx` | `AppStateContext` / state store-derived balances and group state | IMPLEMENTED | Existing React Home is the production surface, not a fixture | Existing component behavior only; no Golden-conformance evidence yet | Empty state exists; approved caught-up/offline/loading semantics not yet proven | UI/presentation model must be reconciled without changing approved hierarchy | BLOCKED |
...
**Result:** no production runtime code was changed in this pass. The first mapped P0 surfaces expose architectural/product-contract conflicts, so fabricating one “easy” UI change would create false progress and make later migration harder. The safe next step is to continue mapping until an actually unblocked Golden slice is found, or implement the smallest explicit enabling seam (for example J04 post-consent group retrieval) with its own tests and security review.

Journey 20 is excluded while it remains unapproved.
