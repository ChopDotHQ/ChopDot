...
export function validateAgentContract(contract, options = {}) {
  ...
  if (contract.task_route) {
    ...
    const expectedWrites = contract.task_route.approval_boundary?.mutation_allowed ? contract.scope?.in_paths ?? [] : [];
    if (digestObject(contract.authority?.allowed_writes ?? []) !== digestObject(expectedWrites)) issues.push(issue('authority.allowed_writes', 'Route-bound write authority must equal the routed task path scope', 'route_authority_mismatch'));
    if (digestObject(contract.requirement_ids ?? []) !== digestObject(contract.task_route.contract_inputs?.requirement_ids ?? []) || digestObject(contract.evaluator?.deterministic_commands ?? []) !== digestObject(contract.task_route.contract_inputs?.deterministic_commands ?? []) || contract.context?.governing_sources?.[0]?.path !== contract.task_route.contract_inputs?.source_path) issues.push(issue('task_route.contract_inputs', 'Route-bound requirements, governing source, and deterministic checks must remain exact', 'route_contract_input_mismatch'));
  }
