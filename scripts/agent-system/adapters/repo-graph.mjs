...
export function createRepoGraphAdapter(options = {}) {
  ...
  return {
    ...
    async read_context(scopeInput = {}, question = '', authorityPolicy = 'exact-root-packet') {
      ...
      const facts = rawFacts.map((entry, index) => ({ fact_id: `fact_repo_graph_${index + 1}`, statement: typeof entry === 'string' ? entry : entry.statement ?? entry.fact ?? JSON.stringify(entry), citation_ids: ['citation_repo_graph_1'], confidence: entry.confidence ?? (staleReasons.length ? 0 : 1) }));
      ...
      return {
        ...
        ...metadata, facts, citations: [citation], source_identities: [source],
        freshness: { observed_at: value.generated_at ?? observedAt, max_age_seconds: value.max_age_seconds ?? 86_400, age_seconds: value.age_seconds ?? 0, status: staleReasons.length ? 'stale' : 'fresh' },
