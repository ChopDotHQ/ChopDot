...
function input(overrides = {}) {
  return { requirement_id: 'REQ-FIXTURE', effect_type: 'other', target: 'fixture://target', intended_payload: { value: 1 }, expected_change: 'The fixture target changes once.', risk: 'low', ...overrides };
}

test('idempotency key is stable over key order', () => {
  const first = effectIdempotencyKey({ run_id: 'run_fixture_12345678', requirement_id: 'REQ-FIXTURE', effect_type: 'other', target: 'x', intended_payload: { a: 1, b: 2 } });
  const second = effectIdempotencyKey({ run_id: 'run_fixture_12345678', requirement_id: 'REQ-FIXTURE', effect_type: 'other', target: 'x', intended_payload: { b: 2, a: 1 } });
...
test('approval-required effect must bind approval identity at plan time', async () => {
  const { contract, runDirectory } = await started();
  await assert.rejects(() => planEffect(runDirectory, contract, input({ approval_required: true })), /bind approval_id/);
});

test('expired approval cannot be approved', async () => {
  const { contract, runDirectory } = await started();
  const approvalId = 'approval_fixture_expired';
  const effect = await planEffect(runDirectory, contract, input({ approval_required: true, approval_id: approvalId, approval_expires_at: '2020-01-01T00:00:00.000Z' }));
...
test('approved effect is consumed when dispatch starts', async () => {
  ...
  const { effect } = await planEffect(runDirectory, contract, input({ approval_required: true, approval_id: approvalId, approval_expires_at: expires }));
