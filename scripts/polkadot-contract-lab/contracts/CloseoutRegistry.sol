...
contract CloseoutRegistry {
    ...
    function _requireAuthorizedLegActor(
        Closeout storage closeout,
        SettlementLeg storage leg
    ) internal view {
        require(
            msg.sender == closeout.creator ||
                msg.sender == leg.payer ||
                msg.sender == leg.payee,
            "not authorized for leg"
        );
    }
    ...
    function recordSettlementProof(
        ...
        _requireAuthorizedLegActor(closeout, leg);
    ...
    function markLegAcknowledged(uint256 closeoutId, uint32 legIndex) external {
        ...
        SettlementLeg storage leg = closeoutLegs[closeoutId][legIndex];
        _requireAuthorizedLegActor(closeout, leg);
        require(leg.status == LegStatus.Proven, "leg not proven");
