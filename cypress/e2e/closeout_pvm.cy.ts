...
describe("PVM closeout", () => {
  ...
  it("retries proof recording from history when payment exists but proof is missing", () => {
    const closeoutId = "12345";
    ...
    seedGuestData([prooflessPot], settlements);

    cy.contains("PVM E2E Pot", { timeout: 20000 }).click({ force: true });
    cy.contains("button", /Settle Up/i, { timeout: 20000 }).click({ force: true });
    cy.contains("button", /Alice/i, { timeout: 20000 }).click({ force: true });
    cy.contains("button", /^History$/i).click({ force: true });

    cy.contains(/Retry proof recording/i, { timeout: 20000 }).click({ force: true });
    cy.contains(/Settlement proof recorded onchain/i, { timeout: 20000 }).should("be.visible");
    cy.contains(/Proof status/i).parent().should("contain.text", "completed");
  });
});
