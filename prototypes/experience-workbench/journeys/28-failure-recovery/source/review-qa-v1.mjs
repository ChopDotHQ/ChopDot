...
for(const req of ['Journey 28','v1.1','window.J28_DEFS','window.J28_ROOTS','window.J28_BOUNDARIES','OP-28-DEMO','result-unknown','reconcile-checking','reconcile-not-found','safe-retry-review','window.J28_TEST_RESET','window.J28_TEST_TRUTH'])if(!html.includes(req))errors.push(`missing static marker: ${req}`);
...
async function reachState(p,id){const proof=pathProof[id];await p.goto(stateUrl('recovery-entry'),{waitUntil:'load'});await p.evaluate(()=>window.J28_TEST_RESET());await waitRendered(p,'recovery-entry');for(const e of proof.steps)if(!await clickTransition(p,e))return false;return(await renderedState(p))===id}
...
async function go(pathTargets){await seq.goto(stateUrl('recovery-entry'),{waitUntil:'load'});await seq.evaluate(()=>window.J28_TEST_RESET());await waitRendered(seq,'recovery-entry');for(const to of pathTargets){const loc=seq.locator(`#content [data-to="${to}"]`).first();if(await loc.count()!==1)return sequence(`path to ${to}`,false,'control missing');await loc.click();await waitRendered(seq,to)}return true}
...
await go(['result-unknown','reconcile-checking','reconcile-not-found']);t=await truth(seq);sequence('no-effect proof authorizes retry',t.status==='not-found'&&t.retryAuthorized===true,JSON.stringify(t));await seq.locator('#content [data-to="safe-retry-review"]').click();await waitRendered(seq,'safe-retry-review');await seq.locator('#content [data-to="retrying"]').click();await waitRendered(seq,'retrying');t=await truth(seq);sequence('replacement starts once and consumes authorization',t.retryCount===1&&t.retryAuthorized===false&&t.status==='retrying',JSON.stringify(t));await seq.locator('#content [data-to="retry-success"]').click();await waitRendered(seq,'retry-success');sequence('replacement verified without second retry',(await truth(seq)).retryCount===1,JSON.stringify(await truth(seq)));
...
await go(['partial-result','partial-reconcile']);body=await bodyText(seq);sequence('partial reconciliation blocks retry',!body.match(/Review safe retry|Start replacement/),body);
await seq.close();
