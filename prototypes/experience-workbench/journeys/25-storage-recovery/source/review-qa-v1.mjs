...
const pathProof={};
for(const id of ids){pathProof[id]=shortestPath(id);if(!pathProof[id])errors.push(`no truthful caller path: ${id}`)}
...
for(const vp of viewports){
  ...
  // Reviewer repair: exact restore facts must remain visible through conflict -> preview -> final review -> confirmation.
  ...
  await p.locator('[data-test-tertiary]').click();
  await p.waitForFunction(()=>location.hash==='#restore-conflict');
  ...
  await p.locator('[data-test-primary]').click();
  await p.waitForFunction(()=>location.hash==='#restore-preview');
  ...
  await p.waitForFunction(()=>location.hash==='#restore-preview');
  await p.locator('[data-test-primary]').click();
  await p.waitForFunction(()=>location.hash==='#restore-ready-to-confirm');
  ...
  await p.locator('[data-test-primary]').click();
  await p.waitForFunction(()=>location.hash==='#restore-confirmation');
  ...
  // Reviewer repair: shell navigation hands off to approved adjacent journey owners rather than bouncing to J25 settings.
  ...
  await assert(`${vp.name} Pots nav interaction reaches Home Golden`,()=>p.evaluate(()=>location.pathname.endsWith('/02-home-orientation/v1.4-golden.html')),true);
...
const summary={
  status:errors.length?'FAILED':'COMPLETE_CANDIDATE_QA_PASSED',journey:'25',factory_generation:'v1.1',
  ...
  reviewer_repairs:{restore_preview_fact_continuity:true,caller_aware_back:true,adjacent_nav_ownership:true,j24_golden_shell_inheritance:true,neutral_cancelled_treatment:true},
  coverage:{registered_states:stateIds.length,owner_boundaries:boundaryIds.length,caller_reachability_proved:reachable,caller_reachability_required:ids.length,direct_rendered_states:ids.length,viewports:viewports.map(v=>`${v.width}x${v.height}`),screenshots:screenshots.length,layout_records:layouts.length,interaction_checks:interactions.length,interaction_passed:interactions.filter(x=>x.pass).length},
  errors,page_errors:pageErrors,console_errors:consoleErrors,external_requests:external,
  claim_boundary:['Deterministic standalone prototype only','No production encryption/key derivation','No real filesystem persistence','No live cloud/provider credentials, sync, retention or deletion','No production database restore','No source authenticity/finality proof','No payment/wallet/signing authority'],
  authority:['Local working copy is distinct from backup/provider copies','Backup artifact is created before destination handoff','Restore is preview-first and preserves newer current facts','Restore confirmation is bound to an explicit deterministic apply set','Secrets/executable authority never round-trip','Unknown backup/save/restore outcomes reconcile before retry','Identity ambiguity is held out, never name-merged','Owner/system boundaries are explicit','TYPO-01 deferred']
};
