...
const emailResult=(s,code='123456')=>M.emailVerificationResult(s,code);
...
s=M.apply(s,'VERIFY_CODE',emailResult(s));eq(s.verified,true);eq(s.verifiedSubject,'email:other@example.com');eq(s.verifiedDestination,'invite');eq(s.verifiedEpoch,s.verificationEpoch);eq(M.verificationCurrent(s),true);
...
// Cross-restart email replay through the ordinary provider/adapter result shape: bare evidence and S1-bound evidence both fail closed in S2.
for(const destination of ['home','invite']){
 let s1=M.initial();if(destination==='invite')s1=M.apply(s1,'INVITE');s1=enterEmail(s1,'dev@example.com');const p1=proof(s1),r1=emailResult(s1);let accepted=M.apply(s1,'VERIFY_CODE',r1);eq(accepted.verified,true,`S1 email result valid for ${destination}`);
 let s2=M.initial();if(destination==='invite')s2=M.apply(s2,'INVITE');s2=enterEmail(s2,'dev@example.com');const p2=proof(s2),r2=emailResult(s2);ne(p2.epoch,p1.epoch,`email ${destination} epoch differs after restart`);ne(p2.request,p1.request,`email ${destination} request identity differs after restart`);eq(p2.challenge,p1.challenge,'challenge counter may repeat but cannot establish freshness');
 const bare=M.apply(s2,'VERIFY_CODE',{code:'123456'});eq(bare.verified,false,`bare email evidence rejected for ${destination}`);eq(bare.route,'email');
 const replay=M.apply(s2,'VERIFY_CODE',r1);eq(replay.verified,false,`stale S1 normal email result rejected for ${destination}`);eq(replay.route,'email');
 const fresh=M.apply(s2,'VERIFY_CODE',r2);eq(fresh.verified,true,`fresh S2 normal email result accepted for ${destination}`);eq(fresh.verifiedEpoch,r2.epoch);
}

// START_OVER is a full reset boundary for both wallet and ordinary email result delivery.
...
s=enterEmail(M.initial(),'dev@example.com');const emailBeforeReset=emailResult(s);s=M.apply(s,'START_OVER');s=enterEmail(s,'dev@example.com');const emailAfterReset=emailResult(s);ne(emailAfterReset.epoch,emailBeforeReset.epoch);ne(emailAfterReset.request,emailBeforeReset.request);let staleAfterReset=M.apply(s,'VERIFY_CODE',emailBeforeReset);eq(staleAfterReset.verified,false);eq(staleAfterReset.route,'email');s=M.apply(s,'VERIFY_CODE',emailAfterReset);eq(s.verified,true);eq(s.verifiedEpoch,emailAfterReset.epoch);
