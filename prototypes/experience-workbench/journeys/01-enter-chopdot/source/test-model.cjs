...
s=M.apply(s,'VERIFY_CODE',emailResult(s,'000000'));eq(s.verified,false);eq(s.route,'code');s.expired=true;s=M.apply(s,'VERIFY_CODE',emailResult(s));eq(s.verified,false);
s=M.apply(s,'RESEND');eq(s.expired,false);eq(s.challenge,2);s=M.apply(s,'VERIFY_CODE',emailResult(s));eq(s.verified,true);eq(s.route,'profile');eq(s.joined,false);
...
s=M.apply(M.initial(),'REAUTH',{destination:'invite'});s=M.apply(s,'EMAIL');s=M.apply(s,'SET_EMAIL',{value:'other@example.com'});s=M.apply(s,'SEND_CODE');s=M.apply(s,'VERIFY_CODE',emailResult(s));eq(s.route,'wrong-account');eq(s.verified,false);eq(s.destination,'invite');
s=M.apply(s,'BACK_TO_EMAIL');s=M.apply(s,'SET_EMAIL',{value:'dev@example.com'});s=M.apply(s,'SEND_CODE');s=M.apply(s,'VERIFY_CODE',emailResult(s));eq(s.route,'ready');eq(s.isNew,false);
