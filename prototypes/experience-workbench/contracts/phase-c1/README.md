...
12. Spend evidence is bound to the exact `{spend_intent_id, operation_id, effect_id, authoritative_effect_ref, adapter/rail identity, amount, asset, MoneyV1 minor units/currency/exponent partition, authorization_version, target digest, policy snapshot, approval snapshot}` plus finality/readback. A proof for one effect is invalid for another even when displayed amount/asset match. Before SPEND-01 conservation is evaluated, every adapter must canonicalize value through the shared exact-money partition (`MoneyV1` or equivalent canonical integer units + currency + exponent); display decimals and any two-decimal fixture are non-authoritative.
...
15. Partial capture, refund and reversal stay on the original operation lineage and the same exact money partition; currency/exponent substitution fails closed.
