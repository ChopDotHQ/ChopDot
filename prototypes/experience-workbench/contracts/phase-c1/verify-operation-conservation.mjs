...
// Contract surface: exact money partitioning is delegated to the shared MoneyV1 primitive.
...
eq(spend.lineage.refund_and_reversal_share_parent_remaing_consumable, true, 'refund and reversal consume the same remaining capture balance');
...
const assertExponent = exponent => assert.ok(Number.isSafeInteger(exponent) && exponent >= 0 && exponent <= 12);
...
const money = ({ decimal, currency, exponent }) => {
  ...
  const minorUnits = decimalToMinorUnits(decimal, exponent).toString();
  return { v: 1, minorUnits, currency, exponent };
};
const samePartition = (left, right) => left.currency === right.currency && left.exponent === right.exponent;
...
const makeOperation = ({
  spend_intent_id = 'sp_1', operation_id = 'op_1', authorized_amount = '100.00', authorized_asset = 'USD',
  authorized_exponent = 2, authorization_version = 1, policy_snapshot_digest = 'policy:p1',
  approval_snapshot_digest = 'approval:a1', execution_state = 'not_started'
} = {}) => ({
  spend_intent_id, operation_id, authorized_amount, authorized_asset, authorization_version,
  authorized_money: money({ decimal: authorized_amount, currency: authorized_asset, exponent: authorized_exponent }),
  policy_snapshot_digest, approval_snapshot_digest, execution_state,
  captures: new Map(), adjustments: new Map(), authoritative_effect_refs: new Set(), authorization_adjustment_ids: new Set(),
  captured_units: 0n, adjusted_units: 0n,
  current_authority: { participant_id: 'participant:owner', capability_id: 'cap:spend-adjust', capability_version: 7, scope: 'spend_authorization_adjustment', revoked: false }
});
...
const validateCommon = (op, effect)) =>
  ...
  present(effect.effect_id) && present(effect.authoritative_effect_ref) &&
  samePartition(op.authorized_money, effect.canonical_money) && moneyMatchesDecimal(effect.canonical_money, effect.amount, effect.asset);
...
const applyCapture = (op, effect) => {
  ...
  if (op.captures.has(effect.effect_id) || op.authoritative_effect_refs.has(effect.authoritative_effect_ref)) return false;
  ...
  op.captures.set(effect.effect_id, { amount_units: amount, remaining_units: amount, canonical_money: effect.canonical_money, authoritative_effect_ref: effect.authoritative_effect_ref });
...
const applyAdjustment = (op, effect) => {
  ...
  if (!validateCommon(op, effect)) return false;
  if (op.adjustments.has(effect.effect_id) || op.authoritative_effect_refs.has(effect.authoritative_effect_ref)) return false;
  ...
  op.adjustments.set(effect.effect_id, { amount_units: amount, kind: effect.kind, parent_capture_effect_id: effect.parent_capture_effect_id, authoritative_effect_ref: effect.authoritative_effect_ref });
  op.authoritative_effect_refs.add(effect.authoritative_effect_ref);
...
const netMoney = op => ({ v: 1, minorUnits: (op.captured_units - op.adjusted_units).toString(), currency: op.authorized_money.currency, exponent: op.authorized_money.exponent });
const effect = (overrides = {}) => ({
  spend_intent_id: 'sp_1', operation_id: 'op_1', effect_id: 'cap_1', authoritative_effect_ref: 'rail:tx:001', kind: 'partial_capture',
  amount: '40.00', asset: 'USD', money_exponent: 2, canonical_money: money({ decimal:'40.00', currency:'USD', exponent:2 }),
  authorization_version: 1, policy_snapshot_digest: 'policy:p1', approval_snapshot_digest: 'approval:a1',
  ...overrides
});
const authAdjustment = (overrides = {}) => ({
  authorization_adjustment_id: 'authadj_001', spend_intent_id: 'sp_1', operation_id: 'op_1', prior_authorization_version: 1, authorization_version: 2,
  authorized_amount: '120.00', authorized_asset: 'USD', money_exponent: 2, canonical_money: money({ decimal:'120.00', currency:'USD', exponent:2 }),
  policy_snapshot_digest: 'policy:p2', approval_snapshot_digest: 'approval:a2', authorizer_participant_id: 'participant:owner',
  authorizer_capability_id: 'cap:spend-adjust', authorizer_capability_version: 7, authorizer_scope: 'spend_authorization_adjustment', authority_proof_ref: 'proof:authadj001',
  ...overrides
});

// Same external effect cannot be replayed under a fresh internal ID.
{
...
// Bound change requires fresh exact current authority and stays in the same MoneyV1 partition.
{
  const op = makeOperation({ authorized_amount: '100.00' });
  ...
  eq(applyAuthorizationAdjustment(op, authAdjustment({ policy_snapshot_digest: 'policy:p1' })), false, 'reused policy snapshot fails');
  eq(applyAuthorizationAdjustment(op, authAdjustment({ approval_snapshot_digest: 'approval:a1' })), false, 'reused approval snapshot fails');
  ...
  eq(applyAuthorizationAdjustment(op, authAdjustment({ canonical_money: { v:1, minorUnits:'12000', currency:'EUR', exponent:2 } })), false, 'authorization currency partition substitution fails closed');
...
// Duplicate refund under a fresh internal ID is rejected.
{
  const op = makeOperation();
  eq(applyCapture(op, effect({ effect_id: 'cap_parent', authoritative_effect_ref: 'rail:cap:parent' })), true, 'parent capture accepted');
  const refund = effect({ effect_id: 'refund_1', authoritative_effect_ref: 'rail:refund:001', kind: 'refund', amount: '10.00', parent_capture_effect_id: 'cap_parent' });
...
// Refund/reversal share one remaining captured balance; cross-partition substitution fails.
{
  const op = makeOperation();
  eq(applyCapture(op, effect({ effect_id: 'cap_parent', authoritative_effect_ref: 'rail:cap:parent' })), true, 'capture accepted');
  eq(applyAdjustment(op, effect({ effect_id: 'refund_1', authoritative_effect_ref: 'rail:refund:001', kind: 'refund', amount: '25.00', parent_capture_effect_id: 'cap_parent' })), true, 'refund consumes part of capture');
  eq(applyAdjustment(op, effect({ effect_id: 'reversal_1', authoritative_effect_ref: 'rail:reversal:001', kind: 'reversal', amount: '20.00', parent_capture_effect_id: 'cap_parent' })), false, 'reversal cannot double-consume refunded value');
  eq(applyAdjustment(op, effect({ effect_id: 'refund_exp', authoritative_effect_ref: 'rail:refund:exp', kind: 'refund', amount: '1.00', money_exponent: 3, parent_capture_effect_id: 'cap_parent' })), false, 'effect exponent substitution fails closed');
  eq(applyAdjustment(op, effect({ effect_id: 'refund_cur', authoritative_effect_ref: 'rail:refund:cur', kind: 'refund', amount: '1.00', asset: 'EUR', parent_capture_effect_id: 'cap_parent' })), false, 'effect currency substitution fails closed');
  eq(formatMoney(netMoney(op)), '15.00', 'partition-substitution rejection preserves exact net');
}
...
// Positive non-2-decimal partition proves SCALE=2 was fixture-only, not product law.
{
  ...
  eq(applyCapture(op, effect({ effect_id:'kwd_cap', authoritative_effect_ref:'rail:kwd:cap', amount:'1.234', asset:'KWD', money_exponent:3 })), true, 'three-decimal partial capture accepted');
  eq(applyAdjustment(op, effect({ effect_id:'kwd_refund', authoritative_effect_ref:'rail:kwd:refund', kind:'refund', amount:'0.234', asset:'KWD', money_exponent:3, parent_capture_effect_id:'kwd_cap' })), true, 'three-decimal refund accepted');
  eq(formatMoney(netMoney(op)), '1.000', 'three-decimal partition conserves exact minor units');
  eq(applyCapture(op, effect({ effect_id:'kwd_bad_exp', authoritative_effect_ref:'rail:kwd:bad-exp', amount:'1.00', asset:'KWD', money_exponent:2 })), false, 'same currency with substituted exponent fails closed');
}

// Positive lineage: legitimate partial captures plus bounded adjustment yield exact net value.
{
  const op = makeOperation({ authorized_amount: '100.00' });
  ...
  eq(applyAdjustment(op, effect({ effect_id:'refund_15', authoritative_effect_ref:'rail:refund:15', kind:'refund', amount:'15.00', parent_capture_effect_id:'cap_30' })), true, 'bounded adjustment accepted');
  ...
  eq(formatMoney(netMoney(op)), '55.00', 'net canonical value equals captures minus adjustments exactly once');
}
