...
try{
  ...
  for(const vp of viewports){
    ...
    const startEmail=async destination=>{await page.evaluate(d=>EntryDemo.fixture(d==='invite'?'invite':'new'),destination);if(destination==='invite')await page.locator('[data-action=EMAIL]').first().click();else await page.locator('[data-action=EMAIL]').first().click();await fillEmail('dev@example.com');await route('code');};
    ...
    // Reviewer 5707911597: exact cross-restart replay family, wallet + email, home + invite.
    for(const destination of ['home','invite']){
      ...
      await page.goto(url,{waitUntil:'load'});await startEmail(destination);const e1=await emailProof();await fillCode('123456');await route('ready');check(await page.evaluate(()=>EntryDemo.get().verified),`S1 email ${destination} proof valid`,vp.name);
      ...
      await page.evaluate(([p])=>EntryDemo.dispatch('VERIFY_CODE',{code:'123456',...p}),[e1]);await route('email');check(!(await page.evaluate(()=>EntryDemo.get().verified)),`stale S1 email ${destination} proof rejected`,vp.name);
      await fillEmail('dev@example.com');await route('code');const e2fresh=await emailProof();check(e2fresh.epoch===e2.epoch,`email ${destination} S2 remains in fresh epoch`,vp.name);await fillCode('123456');await route('ready');check(await page.evaluate(()=>EntryDemo.get().verified),`fresh S2 email ${destination} proof accepted`,vp.name);
    }

    // Reachable START_OVER is also a full-reset boundary; stale proof must remain unusable.
  ...
  const evidence={suite:'phase-c1-j01-subject-binding',reviewer_receipt:5707911597,security_finding:5707441287,canonical_predecessor_sha256:canonicalSha,generated_successor_sha256:digest(fs.readFileSync(artifact)),model_checks:modelEvidence.checks,cross_restart_replay:'wallet+email / home+invite / reload+START_OVER',viewports,assertions:checks.length,screenshot_count:screenshots.length,screenshots,page_console_network_failures:failures,result:'pass'};
