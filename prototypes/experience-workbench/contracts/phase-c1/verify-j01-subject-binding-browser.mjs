...
try{
  ...
  for(const vp of viewports){
    ...
    // Reviewer 5707911597 + Security 5708182785: cross-restart replay family, including the ordinary email result shape.
    for(const destination of ['home','invite']){
      ...
      await page.goto(url,{waitUntil:'load'});await startEmail(destination);const e1=await emailProof();const e1Result=await emailResult();await fillCode('123456');await route('ready');check(await page.evaluate(()=>EntryDemo.get().verified),`S1 email ${destination} normal result valid`,vp.name);
      await page.goto(url,{waitUntil:'load'});await startEmail(destination);const e2=await emailProof();check(e2.epoch!==e1.epoch,`email ${destination} restart epoch differs`,vp.name);check(e2.request!==e1.request,`email ${destination} restart request identity differs`,vp.name);check(e2.challenge===e1.challenge,`email ${destination} reset challenge may collide without conferring freshness`,vp.name);
      const bare=await page.evaluate(()=>EntryModel.apply(EntryDemo.get(),'VERIFY_CODE',{code:'123456'}));check(!bare.verified&&bare.route==='email',`bare email ${destination} result fails closed instead of borrowing S2 binding`,vp.name);
      await page.evaluate(([r])=>EntryDemo.dispatch('VERIFY_CODE',r),[e1Result]);await route('email');check(!(await page.evaluate(()=>EntryDemo.get().verified)),`stale S1 ordinary email ${destination} result rejected`,vp.name);
      await fillEmail('dev@example.com');await route('code');const e2fresh=await emailProof();check(e2fresh.epoch===e2.epoch,`email ${destination} S2 remains in fresh epoch`,vp.name);check(e2fresh.request!==e1.request,`email ${destination} refreshed S2 request stays distinct from S1`,vp.name);await fillCode('123456');await route('ready');check(await page.evaluate(()=>EntryDemo.get().verified),`fresh S2 ordinary email ${destination} result accepted`,vp.name);
    }

    // Reachable START_OVER is a full-reset boundary for wallet and ordinary email result delivery.
    ...
    await page.goto(url,{waitUntil:'load'});await startEmail('home');const emailBeforeReset=await emailResult();await page.evaluate(()=>EntryDemo.dispatch('START_OVER'));await route('welcome');await page.locator('[data-action=EMAIL]').first().click();await fillEmail('dev@example.com');await route('code');const emailAfterReset=await emailProof();check(emailAfterReset.epoch!==emailBeforeReset.epoch,'START_OVER email epoch differs',vp.name);check(emailAfterReset.request!==emailBeforeReset.request,'START_OVER email request differs',vp.name);await page.evaluate(([r])=>EntryDemo.dispatch('VERIFY_CODE',r),[emailBeforeReset]);await route('email');check(!(await page.evaluate(()=>EntryDemo.get().verified)),'START_OVER rejects stale pre-reset ordinary email result',vp.name);await fillEmail('dev@example.com');await route('code');await fillCode('123456');await route('ready');check(await page.evaluate(()=>EntryDemo.get().verified),'START_OVER accepts only fresh post-reset ordinary email result',vp.name);
  ...
  const evidence={suite:'phase-c1-j01-subject-binding',reviewer_receipt:5707911597,security_finding:5707441287,security_followup:5708182785,canonical_predecessor_sha256:canonicalSha,generated_successor_sha256:digest(fs.readFileSync(artifact)),model_checks:modelEvidence.checks,cross_restart_replay:'wallet+email / home+invite / reload+START_OVER / ordinary email adapter result',email_result_correlation:'request+challenge+subject+destination+epoch required; bare code fails closed',viewports,assertions:checks.length,screenshot_count:screenshots.length,screenshots,page_console_network_failures:failures,result:'pass'};
