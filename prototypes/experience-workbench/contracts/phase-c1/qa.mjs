...
for (const vp of viewports) {
  ...
  await page.goto(`${j01}#root`); check(await page.locator('body').getAttribute('data-state')==='root','J01 root state'); await snap('j01-root');
  ...
  await page.goto(`${j01}#root`); await page.click('#invite-entry'); check(await page.locator('body').getAttribute('data-state')==='invite','J01 private invite context reachable'); check((await page.textContent('#invite')).includes('ledger details remain private'),'J01 private-before-join copy'); await snap('j01-invite');
  ...
  // J04 guest happy path: context -> explicit consent -> durable guest participant.
  await page.goto(`${j04}#preview`); check(await page.locator('body').getAttribute('data-state')==='preview','J04 preview state'); check((await page.textContent('#preview')).includes('stay hidden until you join'),'J04 context-before-consent/private-before-join'); await snap('j04-preview');
  await page.click('#review-join'); check(await page.locator('body').getAttribute('data-state')==='consent','J04 explicit consent state'); check((await page.textContent('#consent')).includes('without creating a full account'),'J04 guest join does not require full account'); await snap('j04-consent');
  await page.click('#confirm-guest'); check(await page.locator('body').getAttribute('data-state')==='joined','J04 durable guest created only after consent'); check(await page.textContent('#participant-id')==='grp_alps:p_7f3a','J04 stable participant ID displayed'); await page.click('#group-home'); check(await page.locator('body').getAttribute('data-outcome')==='guest-group-home','J04 guest reaches group home'); await snap('j04-joined');

  // Recovery continuity.
  await page.goto(`${j04}#joined`); await page.click('#simulate-recovery'); check(await page.locator('body').getAttribute('data-state')==='recovery','J04 recovery path reachable'); check((await page.textContent('#recovery')).includes('grp_alps:p_7f3a'),'recovery preserves participant ID'); check((await page.textContent('#recovery')).includes('does not mint a replacement'),'recovery forbids replacement identity'); await snap('j04-recovery');

  // Explicit link success preserves the durable ID.
  await page.goto(`${j04}#joined`); await page.click('#upgrade'); await page.click('#link-match'); check(await page.locator('body').getAttribute('data-state')==='linked','J04 matching proof links identity'); check((await page.textContent('#linked')).includes('grp_alps:p_7f3a'),'link preserves participant ID'); await snap('j04-linked');

  // Mismatch/collision fails unresolved and cannot silently merge.
  await page.goto(`${j04}#joined`); await page.click('#upgrade'); await page.click('#link-mismatch'); check(await page.locator('body').getAttribute('data-state')==='unresolved','J04 mismatch becomes unresolved'); check((await page.textContent('#unresolved')).includes('won’t merge'),'J04 mismatch explicitly refuses merge'); check((await page.textContent('#unresolved')).includes('Guest ledger participation remains intact'),'J04 mismatch preserves guest ledger participation'); await snap('j04-unresolved');

  // Normal account-backed join remains functional.
  await page.goto(`${j04}#preview`); await page.click('#sign-in'); check(await page.locator('body').getAttribute('data-state')==='account','J04 account path reachable'); await page.click('#account-join'); check(await page.locator('body').getAttribute('data-outcome')==='account-group-home','J04 account-backed join works'); check(await page.locator('body').getAttribute('data-participant-state')==='account_backed','J04 account state explicit'); await snap('j04-account');
