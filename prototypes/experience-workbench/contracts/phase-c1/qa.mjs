...
for (const vp of viewports) {
  ...
  await nav(`${j04}#joined`); await page.click('#upgrade'); await page.click('#link-match'); check(await page.locator('body').getAttribute('data-state')==='linked','J04 matching proof links identity'); check((await page.textContent('#linked')).includes('grp_alps:p_7f3a'),'link preserves participant ID'); await snap('j04-linked');
