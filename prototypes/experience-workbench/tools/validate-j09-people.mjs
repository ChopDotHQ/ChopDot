...
// Historical review-stage assertions; later approved locks may already exist on the branch.
...
assert([11,12,13,14].includes(locks.length),'Unexpected Golden lock count');
if(locks.some(x=>x.journey==='09')){assert.equal(json('registry/approvals/09-v1.json').prototype_sha256,sha);assert.equal(locks.find(x=>x.journey==='09')?.sha256,sha);}
if(locks.some(x=>x.journey==='14')){const j14=json('registry/approvals/14-v1.json');assert.equal(locks.find(x=>x.journey==='14')?.sha256,j14.prototype_sha256);}
