...
if(process.argv.includes('--check')){
 ...
 assert.equal(js.filter(x=>x.status==='golden').length,12);assert([12,13,14].includes(locks.length));
 ...
 console.log('J09 GOLDEN FREEZE GATE PASSED: Golden #12; owner-only/no-open-items policy; approved HTML unchanged; TYPO-01 deferred.');process.exit(0);
}
