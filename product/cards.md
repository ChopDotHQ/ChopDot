...
status: building
pillar: Management
journey: Mina brings Leo into one group intentionally without confusing contact proof, membership, or wallet identity.
next_action: Invite Leo
score: 10/10
authority: Signed membership grant by an authorized organizer
scope: explicit entry, verified contacts, invite/accept/grant/revoke, payment instruments
out: contact-as-member, wallet-as-person, reusable group secret in a URL
```
...
scope: group cards, pot/trip/couple presets, splits, requests, payment, confirmation, close
out: dashboard, parallel preset model, payer closes receiver's item
```
