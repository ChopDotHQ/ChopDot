export function validateHardening(core, graph, frozen, oracle) {
  ...
  const has=(a,x)=>Array.isArray(a)&&a.includes(x), fail=(id,msg)=>errors.push({id,msg}), integration=new Map((frozen.accepted_integration?.gate_a?.inherited_integration_constraints||[]).map(x=>[x.id,x])), recoveries=new Set((frozen.authority_recoveries||[]).filter(x=>x.status==='resolved').map(x=>x.id)), gaps=new Set((core.known_gaps||[]).map(x=>x.id));
  ...
  // Read models are never canonical write targets. Invalidations target read models only.
  ...
  // Every declared effect guard must actually guard every operation named by its law.
  for(const l of core.laws.filter(x=>x.constraint?.kind==='effect_guard_required')){
    for(const id of l.constraint.operations||[])if(!has(op(id)?.guards,l.constraint.guard))fail('GUARD-COVERAGE',id+' missing guard '+l.constraint.guard);
  }

  // Read projections do not own mutations.
  ...
  // Review dependent-state text must agree with the machine law.
  ...
  if(review?.kind==='successful_edit_resets_current_reviews'){
    if(!effect||effect.law_ref!=='LAW-EXP-REVIEW-01'||!effect.effect.includes('needs_review_again')||!effect.effect.includes('accepted persistence'))fail('REVIEW-EFFECT-CONSISTENCY','expense.edit dependent review effect disagrees with LAW-EXP-REVIEW-01');
  }
...
export function evaluateExpenseContract(core,frozen,fixture){
  ...
  const expectedIds=normalizeIds(fixture.selected_participants),actualIds=normalizeIds(ids);
  if(JSON.stringify(expectedIds)!==JSON.stringify(actualIds))failures.push('LAW-EXP-03');
