...
'- Independently semantic detections: **'+a.total.semantic_detected+' / '+a.total.applicable+' ('+a.total.semantic_score+'%)**',
'- Freeze-seal-only detections: **'+a.total.freeze_seal_only+'**',
'- Structural freeze seals: **'+Object.entries(c.stage_5_2?.seals||{}).map(([k,v])=>k+'='+v).join(', ')+'**',
'- Authored/executable V1 blobs sealed: **'+(c.stage_5_2?.authored_blobs?.matched??0)+' / '+(c.stage_5_2?.authored_blobs?.total??0)+'**','',
'## Blast-radius probes','',
'Model: **'+b.model+'**','',
