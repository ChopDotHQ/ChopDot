...
console.log(JSON.stringify({stage:5,reconstruction_status:c.status,golden_pieces:c.golden_pieces.total,unjustified_unmapped:c.golden_pieces.unjustified_unmapped,operation_witnesses:c.operation_witnesses.witnessed+'/'+c.operation_witnesses.total,required_states:c.required_states.resolved+'/'+c.required_states.total,events:c.event_vocabulary.bound_events+'/'+c.event_vocabulary.distinct_events,mutation_score:m.total.score,closure_status:closure,error_count:c.errors.length,errors:c.errors.slice(0,50),result:closure==='PASS'?'PASS':'FAIL'},null,2));
