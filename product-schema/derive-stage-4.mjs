...
writeFileSync(join(out,'completeness-report.json'),JSON.stringify(x.report,null,2)+'\n');
...
const lines=['# Generated Gate B Construction Packet','',
  ...
  '## Resolved authority interpretations','',...p.resolved_authority_interpretations.filter(x=>(x.journeys||[]).some(j=>p.construction_order.includes(j))).map(x=>'- **'+x.id+'** — '+x.rule),'',
