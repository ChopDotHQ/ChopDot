...
'## Approved post-Golden product decisions','',...(p.approved_product_decisions.length?p.approved_product_decisions.map(x=>'- **'+x.id+'** — '+x.status+(x.clarifies?'; clarifies '+x.clarifies:(x.resolves?'; resolves '+x.resolves:''))):['- None']),'',
'## Recorded Golden impacts','',...(p.golden_impacts?.length?p.golden_impacts.map(x=>'- **'+x.id+'** — J'+x.journey+' `'+x.state+'`: '+x.rule):['- None']),'',
