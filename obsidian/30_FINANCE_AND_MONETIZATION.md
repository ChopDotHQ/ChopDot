# Finance And Monetization

## Today

- primary model: subscriptions
- buyer: organizer, community admin, SMB/provider operator
- participant remains free in the base case
- no reliance on custody spread, yield, or token economics

## Tomorrow

- stronger subscriptions
- premium trust / proof actions
- builder/API pricing
- provider workflow pricing

## Finance status

- infra cost is not the main problem
- people cost and operating shape are the main problem
- the current base case is not self-sustaining by year 5

## Key docs

- [Financial Overview](../docs/finance/CHOPDOT_FINANCIAL_OVERVIEW_2026.md)
- [Financial Model Workbook](../docs/finance/CHOPDOT_FINANCIAL_MODEL_2026.xlsx)
- [Monetization Model](../docs/finance/MONETIZATION_MODEL_TODAY_VS_TOMORROW.md)
- [Protocol And Layer Monetization Map](../docs/finance/PROTOCOL_AND_LAYER_MONETIZATION_MAP.md)
- [Launch Perimeter And Survivability Plan](../docs/LAUNCH_PERIMETER_AND_SURVIVABILITY_PLAN.md)
