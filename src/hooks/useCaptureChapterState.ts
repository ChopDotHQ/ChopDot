...
type UseChapterStateResult = {
  ...
  recordReceivedPayment: (input: { legId: string; payerMemberId: string; creditorMemberId: string }) => Promise<void>;
  close: () => Promise<void>;
  refresh: () => Promise<void>;
};

export function useCaptureChapterState({
  potId,
  potService,
  currentMemberId,
  currentMemberName,
  ...
  const recordReceivedPayment = useCallback(
    async (input: { legId: string; payerMemberId: string; creditorMemberId: string }) => {
      if (!chapter) {
        throw new Error('Chapter not loaded');
      }

      const marked = markLegPaid(chapter, {
        payerMemberId: input.payerMemberId,
        legId: input.legId,
      });
      const confirmed = confirmLeg(marked, {
        creditorMemberId: input.creditorMemberId,
        legId: input.legId,
      });
