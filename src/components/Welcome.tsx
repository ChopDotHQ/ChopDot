import {ArrowRight, Camera, ReceiptText, UserRound} from 'lucide-react';
import {useState} from 'react';

export function Welcome({onGuest, onScanReceipt, onUseProductAccount}: {onGuest: () => void; onScanReceipt: () => void; onUseProductAccount?: () => Promise<void>}) {
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[#f7f6f4] px-6 py-7 text-gray-950 transition-colors dark:bg-gray-950 dark:text-white sm:px-7">
      ...
      <section className="mt-10" aria-labelledby="welcome-title">
        ...
        <p className="mt-7 text-sm font-semibold text-[#c40068] dark:text-[#ff65b5]">You paid. Catch it now.</p>
        ...
        <p className="mt-4 max-w-[18rem] text-[15px] leading-6 text-gray-600 dark:text-gray-300">
          Take a photo or import one. You review everything before it reaches the group.
        </p>
      </section>
      ...
      {accountState === 'error' && <p role="alert" className="mt-2 text-center text-sm font-medium text-red-700">Your account did not connect. You can retry, scan now, or continue as guest.</p>}

      <p className="mt-3 text-center text-xs leading-5 text-gray-600 dark:text-gray-300">Scanning makes a local draft. Nothing is shared yet.</p>
    </main>
  );
}
